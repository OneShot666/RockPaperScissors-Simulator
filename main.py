...
# ... [v0.1.1] Add menus + end of simulation if only one type remaining (add auto-end option)
# ! [v0.1.2] Add sounds (when entity been beaten) + chill music + mutation option (can change type randomly)
# ! [v0.1.3] Add graphics + saves result + scoreboard -> create datamanager class
...
# ! [v1.0.0] Make the code into a .exe
class RPSSimulator:                                                             # Main class
    def __init__(self, name=None):
        ...
        self.timer_one_sec = TimerManager(1)                                    # To avoid spamming (button or screen)
        # Sounds data
        self.volume = 0.5
        self.Sounds = []
        ...
        if not self.simulating:                                                 # Main menu
            ...
            if self.pausing:                                                    # Display in front of almost everything...
                ...
                self.EntityManager.move_entities(self.screen_size, self.mouse,
                    self.is_map_borders, self.is_infinity_map)
        ...
        if not self.is_ui_hide:
            for entity in self.EntityManager.Entities:                          # In another loop to avoid overlay beyond entities
                if entity.is_mouse_over(self.mouse):
                    ...
                    if self.timer_entity_infos.check_loop_end(self.current_time) > 0:
                        self.timer_entity_infos.pause(self.current_time)
                    elif self.timer_entity_infos.check_loop_end(self.current_time) < 0:
    ...
    def open_parameter(self):
        ...
        if self.simulating:
            self.pausing = True                                             # Pause simulation when open parameters
            self.timer_simulation.pause(self.current_time)
    ...
    def display_parameters_screen(self):                                        # Display parameters screen
        ...
        # First column
        self.is_display_turn, _ = self.TextManager.display_parameter(*trio,
            0.15, 0.3, self.is_display_turn, "Display number of turns : ")
        if self.is_display_turn:                                                # Select pause for each turn
            s = "s" if self.timer_turn.duration > 1 else ""
            self.TextManager.display_message(self.screen, 0.15, 0.35,
        ...
        self.is_map_borders, as_change = self.TextManager.display_parameter(*trio,
            0.15, 0.44, self.is_map_borders, "Entities stay on screen : ", descr)
        ...
        self.is_infinity_map, as_change = self.TextManager.display_parameter(*trio,
            0.15, 0.5, self.is_infinity_map, "Screen is toroidal : ", descr)
        if as_change and self.is_infinity_map:
            self.is_map_borders = False

        descr = "When on, display quantity of entity in percent amount"
        self.is_nb_in_percent, _ = self.TextManager.display_parameter(*trio,
            0.15, 0.57, self.is_nb_in_percent, "Quantity is in percent : ", descr)

        descr = "When on, display ui in front of entities. When off, display it behind"
        self.is_ui_on_front, _ = self.TextManager.display_parameter(*trio,
            0.15, 0.64, self.is_ui_on_front, "UI is on front : ", descr)

        descr = "When on, do not display ui"
        self.is_ui_hide, _ = self.TextManager.display_parameter(*trio,
            0.15, 0.7, self.is_ui_hide, "UI is hide : ")
        ...
        self.EntityManager.balanced, _ = self.TextManager.display_parameter(*trio,
            0.15, 0.77, self.EntityManager.balanced, "Balanced : ", descr)

        # Second column
        radius = self.EntityManager.Entities[0].range
        ...
        self.EntityManager.is_entity_range, _ = self.TextManager.display_parameter(*trio,
            0.55, 0.3, self.EntityManager.is_entity_range, "Entity have range : ", descr)

        nb = self.EntityManager.nb_entities
        nb_max = self.EntityManager.nb_max
        self.TextManager.display_message(self.screen, 0.55, 0.35, f"Quantity : {nb}", "middle", "left")
        if self.TextManager.display_button(*trio, "-", (0.75, 0.35, 0.025, 0.04), "lightgrey", True) and nb > 0:
            self.EntityManager.reduce_nb_entities()
        if self.TextManager.display_button(*trio, "+", (0.8, 0.35, 0.025, 0.04), "lightgrey", True) and nb < nb_max:
            self.EntityManager.increase_nb_entities()
        percent = (nb / nb_max) * (self.screen_size[0] * 0.28)
        pos = (self.screen_size[0] * 0.55, self.screen_size[1] * 0.4, percent, 10)
        pygame.draw.rect(self.screen, self.borders_color, pos, border_radius=int(pos[3] * 0.5))
        ...
        self.EntityManager.is_smart_entity, as_change = self.TextManager.display_parameter(*trio,
            0.55, 0.45, self.EntityManager.is_smart_entity, "Entities are smart : ", descr)
        if as_change:
            if self.EntityManager.is_smart_entity:
        ...
        self.EntityManager.is_follow_mouse, _ = self.TextManager.display_parameter(*trio,
            0.55, 0.5, self.EntityManager.is_follow_mouse, "Entities follow mouse : ", descr)
        if self.EntityManager.is_follow_mouse:                                  # Select following entity
            self.TextManager.display_message(self.screen, 0.55, 0.55, f"Entity to follow : "
        ...
        self.EntityManager.is_spawn_with_click, _ = self.TextManager.display_parameter(*trio,
            0.55, 0.7, self.EntityManager.is_spawn_with_click, "Spawn entity on click : ", descr)
        if self.EntityManager.is_spawn_with_click:                              # Select entity to spawn
            ...
            self.TextManager.display_message(self.screen, 0.55, 0.75, text, "middle", "left")
    ...
    def display_entities_button(self, x, y, image_size=50, gap=10, current_entity_name=None):
        ...
        for i, entity in enumerate(Each):
            ...
            border_color = self.TextManager.Colors["gold"] if current_entity_name and \
